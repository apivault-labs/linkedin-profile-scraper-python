"""Thin synchronous client for the LinkedIn Profile Scraper Apify Actor."""
...
from .exceptions import ActorRunError, ActorTimeoutError, AuthenticationError, LinkedInProfileScraperError

ACTOR_ID = "apivault_labs~linkedin-profile-scraper-no-cookies"
APIFY_API_BASE = "https://api.apify.com/v2"
TERMINAL_FAIL = {"FAILED", "TIMED-OUT", "ABORTED"}


class LinkedInProfileScraperClient:
    ...
    def __init__(self, api_token: str | None = None, timeout: int = 600,
        ...
        self.session.headers.update({
            ...
            "User-Agent": "linkedin-profile-scraper-python/0.1.0",
        })
    ...
    @staticmethod
    def estimate_cost(result_count: int, price_per_1000: float = 2.0) -> float:
    ...
    def _start(self, payload: dict[str, Any], timeout_secs: int) -> str:
        try:
            ...
            raise LinkedInProfileScraperError(f"Could not start Actor run: {exc}") from exc
    ...
    def _wait(self, run_id: str) -> dict[str, Any]:
        ...
        while True:
            try:
                ...
                raise LinkedInProfileScraperError(f"Could not poll Actor run: {exc}") from exc
            if response.status_code >= 400:
    ...
    def _dataset(self, dataset_id: str) -> list[dict[str, Any]]:
        try:
            ...
            raise LinkedInProfileScraperError(f"Could not download Dataset: {exc}") from exc
        if response.status_code >= 400:
